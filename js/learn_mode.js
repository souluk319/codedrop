...
const LearnMode = (() => {
    ...
    function chatTitleLabel() {
        return ui.chatEngine && ui.chatEngine.value === 'openai' ? 'GPT 5.4 MINI' : 'KUGNUS AI';
    }
    ...
    function loadChatHistory() {
        try {
            session.chat = normalizeChatEntries(JSON.parse(localStorage.getItem(chatStorageKey())));
        } catch (e) {
            session.chat = [];
        }
    }
